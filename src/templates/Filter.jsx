...
class Filter extends Component{
    ...
    componentDidMount(){
        if(this.props["defaultValue"]){
            window.filter = this;
            this.setData(this.props["defaultValue"]);
        }
        this.getData();
    ...
    create_items(){
        ...
        for(let i in items){
            let item = items[i];
            let ele ;
            if(items[i].type==="range_date"||items[i].type==="range_time"||items[i].type==="range_date_notime"){
                ...
                <div className="item" key={i} style={{visibility:i==="blank"?"hidden":"visible"}}>
                    ...
                    <div className="element">
                        <FormItem {...formItemLayout}>
                            {getFieldDecorator(i,{initialValue:defaultValue[i]||undefined})(ele)}
                        </FormItem>
                    </div>
                </div>
            )
    ...
    getData(){
        ...
        for(let f in form_data){
            ...
            if(form_data[f] instanceof Array){
	...
	render(){
		...
		return (
            <Form className="filter-bar">
                ...
                <div className="btns">
                    <Button type="primary" onClick={this.getData.bind(this)}>查询</Button>&emsp;
