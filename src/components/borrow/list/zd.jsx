...
class Borrow extends Component{
    ...
    componentWillMount(){
        this.columns = [
...

            {
                ...
                // width:100,
                render: (data) => (
                    data.key==="合计"?null:<Permissions size="small" server={global.AUTHSERVER.mgnt.key} permissions={global.AUTHSERVER.mgnt.access.contract_detail} tag="button" onClick={()=>(this.detail(data))} src={'/jk/list/zd/detail?borrowerId='+data.borrowerId+"&contract_no="+data.contractNo+"&contractId="+data.contractId}>查看</Permissions>
                )
            }
        ];
    ...
    get_select(){
        ...
        axios_loanMgnt.post("/manage/util/getLoanAppOptions",{labelType:"BUSINESS",labelName:"助贷业务",usage:"CONTRACT_LIST"}).then(e=>{
            if(!e.code){
                var domain=[];
                for(var i in e.data){
        ...
        axios_loanMgnt.post("/manage/util/getProductOptions",{labelType:"BUSINESS",labelName:"助贷业务",usage:"CONTRACT_LIST"}).then(e=>{
            if(!e.code){
                var appKey=[];
                for(var i in e.data){
    ...
    detail(data){
        window.open('/jk/list/zd/detail?borrowerId='+data.borrowerId+"&contract_no="+data.contractNo+"&contractId="+data.contractId+"&labelName=助贷业务&domainNo="+data.domainNo);
    }
