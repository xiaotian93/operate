...
import { jyd_list_approve0 , jyd_deny0 , jyd_approve0 } from '../../../ajax/api';
...
class check_hs extends Component{
    ...
    componentWillMount(){
        this.columns = [
            ...
            },
            {
                ...
                // fixed: 'right',
                render: (data) => {
                    var btn=[<Permissions server={global.AUTHSERVER.ygd.key} tag="button" type="primary" size="small" onClick={()=>(this.approved([data.orderNo],true))} permissions={global.AUTHSERVER.ygd.access.jyd_approve0}>通过</Permissions>,<Permissions server={global.AUTHSERVER.ygd.key} tag="button" type="danger" size="small" onClick={()=>(this.approved([data.orderNo],false))} permissions={global.AUTHSERVER.ygd.access.jyd_retreat0}>驳回</Permissions>,<Permissions server={global.AUTHSERVER.ygd.key} tag="button" size="small" onClick={()=>(this.detail([data.orderNo]))} permissions={global.AUTHSERVER.ygd.access.jyd_approve0_detail} src={'/db/check/jyd/detail?orderNo='+data.orderNo+'&audit=first&type=check'}>查看</Permissions>]
    ...
    get_list(page_no=1,filter={}){
        ...
        axios_ygd.post(jyd_list_approve0,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    detail(orderNo){
        window.open('/db/check/jyd/detail?orderNo='+orderNo+'&audit=first&type=check');
    }
    ...
    approve_post(orderNo,approved,comment){
        ...
        let api=approved?jyd_approve0:jyd_deny0;
