...
import { jyd_list_approve1 , jyd_deny1 , jyd_approve1 } from '../../../ajax/api';
...
class check_hs extends Component{
    ...
    componentWillMount(){
        this.columns = [
            ...
            },
            {
                ...
                //fixed: 'right',
                render: (data) => {
                    var btn=[<Permissions server="bmd-yuangongdai" tag="button" type="primary" size="small" onClick={()=>(this.approved([data.orderNo],true))} permissions={global.AUTHSERVER.ygd.access.jyd_approve1}>通过</Permissions>,
                    <Permissions server="bmd-yuangongdai" tag="button" type="danger" size="small" onClick={()=>(this.approved([data.orderNo],false))} permissions={global.AUTHSERVER.ygd.access.jyd_retreat1}>驳回</Permissions>,
                    <Permissions server="bmd-yuangongdai" tag="button" size="small" onClick={()=>(this.detail(data.orderNo))} permissions={global.AUTHSERVER.ygd.access.jyd_approve1_detail} src={'/db/review/jyd/detail?orderNo='+data.orderNo+'&audit=first&type=review'}>查看</Permissions>];
    ...
    get_list(page_no=1,filter={}){
        ...
        axios_ygd.post(jyd_list_approve1,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    detail(orderNo){
        window.open('/db/review/jyd/detail?orderNo='+orderNo+'&audit=first&type=review');
    }
