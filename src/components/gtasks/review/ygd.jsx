...
import { ygd_list_approve1 , cxfq_product_list , cxfq_business_list , ygd_deny1 , ygd_approve1 , cxfq_export_excel } from '../../../ajax/api';
...
class check_hs extends Component{
    ...
    componentWillMount(){
        this.columns = [
            ...
            },
            {
                ...
                //fixed: 'right',
                render: (data) => {
                    var btn=[<Permissions server="bmd-yuangongdai" tag="button" type="primary" size="small" onClick={()=>(this.approved([data.orderNo],true))} permissions={global.AUTHSERVER.ygd.access.ygd_approve1}>通过</Permissions>,<Permissions server="bmd-yuangongdai" tag="button" type="danger" size="small" onClick={()=>(this.approved([data.orderNo],false))} permissions={global.AUTHSERVER.ygd.access.ygd_retreat1}>驳回</Permissions>,<Permissions server="bmd-yuangongdai" tag="button" size="small" onClick={()=>(this.detail(data.orderNo))} permissions={global.AUTHSERVER.ygd.access.ygd_approve1_detail} src={'/db/review/ygd/detail?orderNo='+data.orderNo+'&audit=first&type=review'}>查看</Permissions>];
    ...
    get_list(page_no=1,filter={}){
        ...
        axios_ygd.post(ygd_list_approve1,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    detail(orderNo){
        window.open('/db/review/ygd/detail?orderNo='+orderNo+'&audit=first&type=review');
    }
    ...
    approve_post(orderNo,approved,comment){
        ...
        let api=approved?ygd_approve1:ygd_deny1;
