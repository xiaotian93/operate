...
import {axios_bd,axios_cxfq} from '../../../../ajax/request';
import {bd_company_list,bd_product_list,bd_customer_list,tbd_list_hs,patch_bd_prepare,patch_bd,deny_archive} from '../../../../ajax/api';
...
class Borrow extends Component{
    ...
    componentWillMount(){
        ...
        this.filter = {
            time:{
                name:"订单日期",
                type:"range_date",
                feild_s:"createStartTime",
                feild_e:"createEndTime",
            ...
            archive_status :{
                ...
                type:"select",
                placeHolder:"全部",
                values:[{name:"待补录",val:0},{name:"已补录",val:1},{name:"补录中",val:2},{name:"补录驳回",val:-1}]
            },
            productName :{
                name:"产品名称",
                type:"select",
        ...
        this.columns=[
            ...
            },
            {
                ...
                render:(e)=>{
                    return (<div>
                        <Button type="primary" size="small" onClick={()=>{this.prepare(e.orderId)}} disabled={this.state.button} >补录</Button>
                    </div>)
    ...
    get_list(page_no,filter={}){
        ...
        axios_bd.post(tbd_list_hs,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    // getSelect(){
    //     axios_bd.get(company_list).then(e=>{
    //         this.setState({company_list:this.filterValue(e.data)})
    //     });
    //     axios_bd.get(product_list).then(e=>{
    //         this.setState({product_list:this.filterValue(e.data)})
    //     });
    //     axios_bd.get(get_merchant_list).then(e=>{
    ...
    prepare(id){
        ...
        axios_bd.get(patch_bd_prepare+"?orderId="+id).then(e=>{
            if(!e.code){
                var bdArr=[];
