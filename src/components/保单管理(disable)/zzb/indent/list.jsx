...
class Borrow extends Component{
        ...
        this.columns=[
            ...
            },
            {
                ...
                render:(e)=>{
                    ...
                    return (<div>
                        <Permissions size="small" onClick={()=>{this.detail(e.id)}} style={{marginRight:"5px"}} server={global.AUTHSERVER.insurance.key} permissions={global.AUTHSERVER.insurance.access.zzb_insurance_detail} tag="button" src={"/bd/indent/zzb/detail?id="+e.id}>查看</Permissions>
                        <Permissions type="success" size="small" onClick={()=>{this.start_crawl(e.id)}} disabled={this.state.button} server={global.AUTHSERVER.insurance.key} permissions={global.AUTHSERVER.insurance.access.zzb_insurance_crawler} tag="button" style={{ marginTop: "5px" }}>爬取</Permissions>
                        {
                            e.status?null:btn
                        }
    ...
    getSelect(){
        axios_bd.get(bd_company_list+'?businessName=zzb').then(e=>{
            this.setState({company_list:this.filterValue(e.data)})
        });
        axios_bd.get(bd_product_list+'?businessName=zzb').then(e=>{
            this.setState({product_list:this.filterValue(e.data)})
        });
        axios_bd.get(bd_customer_list+'?businessName=zzb').then(e=>{
    ...
    detail(id){
        bmd.navigate("/bd/indent/zzb/detail?id="+id);
    }
    tb(id){
        browserHistory.push("/bd/indent/zzb/tb?id="+id);
    }
    pd(id){
        browserHistory.push("/bd/indent/zzb/pd?id="+id);
    }
    render (){
        let pagination = {
