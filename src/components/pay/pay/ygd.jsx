...
import { get_wait_pay_list , cxfq_product_list , cxfq_business_list , ygd_deny1 , ygd_approve1 , cxfq_export_excel } from '../../../ajax/api';
...
class check_hs extends Component{
        ...
        this.columns = [
            ...
            },
            {
                ...
                render: (data) => {
                    return (
                        <span>
                            <Permissions size="small" onClick={()=>(this.detail(data.orderNo))} server={global.AUTHSERVER.ygd.key} tag="button" permissions={global.AUTHSERVER.ygd.access.ygd_wait_pay_detail} src={'/zf/pay/ygd/detail?orderNo='+data.orderNo+'&audit=first&type=pay'}>查看</Permissions>
                        </span>
                    )}
            }
        ];
    ...
    get_list(page_no=1,filter={}){
        ...
        axios_ygd.post(get_wait_pay_list,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    detail(orderNo){
        window.open('/zf/pay/ygd/detail?orderNo='+orderNo+'&audit=first&type=pay');
    }
