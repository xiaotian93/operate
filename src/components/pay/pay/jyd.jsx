...
import { get_wait_pay_list_jyd , jyd_deny1 , jyd_approve1 } from '../../../ajax/api';
...
class check_hs extends Component{
        ...
        this.columns = [
            ...
            },
            {
                ...
                render: (data) => {
                    return (
                        <span>
                            <Permissions size="small" onClick={()=>(this.detail(data.orderNo))} server={global.AUTHSERVER.ygd.key} tag="button" permissions={global.AUTHSERVER.ygd.access.jyd_wait_pay_detail} src={'/zf/pay/jyd/detail?orderNo='+data.orderNo+'&audit=first&type=pay'}>查看</Permissions>
                        </span>
                    )}
            }
        ];
    ...
    get_list(page_no=1,filter={}){
        ...
        axios_ygd.post(get_wait_pay_list_jyd,rqd).then((data)=>{
            var list=data.data;
            this.setState({
    ...
    detail(orderNo){
        window.open('/zf/pay/jyd/detail?orderNo='+orderNo+'&audit=first&type=pay');
    }
