...
import { capital_bl_account_edit } from '../../../ajax/api';
...
class Account extends Component {
    ...
    componentWillMount() {
        ...
        this.columns = [
            ...
            },
            {
                ...
                operate: (data) => {
                    let content = [
                        <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" key="edit" size="small" type="primary" onClick={() => { this.edit_account(data) }}>编辑</Permissions>,
                        <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" key="edit" size="small" type="primary" onClick={() => { this.detail_account(data) }}>账户信息</Permissions>,
                        <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" key="show" size="small" onClick={() => { this.get_detail(data) }}>账户明细</Permissions>
                    ]
    ...
    get_detail(data) {
        bmd.redirect("/zj/blaccount/detail?accountId=" + data.accountId)
    }
    detail_account(data) {
        bmd.redirect("/zj/blaccount/info?accountId=" + data.accountId)
    }
    edit_account(data) {
        bmd.redirect("/zj/blaccount/edit?accountId=" + data.accountId)
    }
    cancelModal() {
        this.setState({
            modalVisible: false,
    ...
    accountSubmit(data) {
        axios_zj.post(capital_bl_account_edit, data).then(res => {
            this.cancelModal();
            this.get_list();
        })
    }
    ...
    requestList(param) {
        return new Promise((resolve, reject) => {
            axios_zj.post("/bmd_gj_accounting/balance/all").then(data => {
