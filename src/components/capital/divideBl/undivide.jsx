...
import { capital_bl_undivide, capital_bl_confirm, capital_bl_hangup } from '../../../ajax/api';
...
class Undivide extends Component {
    ...
    listRequestor(rqd) {
        this.page = rqd.page;
        return axios_zj.post(capital_bl_undivide, { ...rqd, type: this.state.detail.type });
    }
    ...
    confirm_request(datas) {
        ...
        axios_zj.post(capital_bl_confirm, rqd).then(data => {
    ...
    // 分账挂起
    hangup_divide(id) {
        axios_zj.post(capital_bl_hangup, { accountingId: id }).then(data => {
            this.get_list(1, this.state.filter);
        })
    }
    render() {
                ...
                <ListCtrl {...listProps}>
                    <div style={{ width: "100%" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "10px" }}>
                            ...
                            <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} type="primary" onClick={(e) => { this.confirm_divide(this.state.allSelectedRows) }} tag="button">确认分账</Permissions>
                        </div>
                    </div>
                </ListCtrl>
            </div>
