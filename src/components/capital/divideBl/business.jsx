...
import { capital_bl_business, capital_bl_business_deny, capital_bl_account_stats } from '../../../ajax/api';
...
class Business extends Component {
    ...
    componentDidMount() {
        this.get_list();
        // this.capital_bl_account_stats();
    }

    // 获取统计
    capital_bl_account_stats() {
        axios_zj.post(capital_bl_account_stats).then(data => {
            let source = [];
            source.push({
                key: "title",
                "income": {
    ...
    get_list(pageNo = 1) {
        ...
        axios_zj.post(capital_bl_business, rqd).then((d2) => {
            this.setState({
                d1: d2.data.list,
                total: d2.data.total,
    ...
    divide_confirm(details) {
        ...
        page_go("/zj/blbusiness/undivide?detail=" + encodeURI(JSON.stringify(detail)));
    }
    ...
    divide_refuse() {
        ...
        axios_zj.post(capital_bl_business_deny, rqd).then(data => {
