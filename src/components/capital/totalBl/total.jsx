...
import { capital_bl_account_detail, capital_bl_account_stat, capital_bl_account_web_add, capital_bl_account_web_sub, capital_bl_account_total, capital_bl_account_export, capital_hangup, capital_bl_account_user_list } from '../../../ajax/api';
import { host_zj, page } from '../../../ajax/config';
import { bmd } from '../../../ajax/tool';
import Permissions from '../../../templates/Permissions';
import ComponentRoute from '../../../templates/ComponentRoute';
import ListCtrl from '../../../controllers/List';
import AccountCtrl from '../../../request/capital/AccountBl';
...
class Detail extends Component {
        ...
        this.columns = [
            ...
            },
            {
                ...
                render: data => {
                    ...
                    if (data.remainAmount < data.amount) return <Permissions key="show" size="small" onClick={(e) => { this.element_show(data) }} server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" src={"/zj/bltotal/element"}>查看</Permissions>
                }
            }
        ];
    ...
    get_total_info(param = {}) {
        axios_zj.post(capital_bl_account_stat, param).then((data) => {
            let state = {
                income: data.data['收入'],
                expend: data.data['支出'],
    ...
    get_total_money(param = {}) {
        ...
        axios_zj.post(capital_bl_account_total, { ...account }).then(data => {
            this.setState({
                loading_total: false,
    ...
    table_export() {
        ...
        let params = "?" + str.join("&");
        window.open(host_zj + capital_bl_account_export + params);
    }

    // 查看成分
    element_show(data) {
        bmd.navigate("/zj/bltotal/element?id=" + data.id + "&account_name=" + AccountCtrl.mapName(data.accountId))
    }
    ...
    person_charge(param) {
        ...
        axios_zj.post(capital_bl_account_web_add, rqd).then(data => {
            this.getList();
    ...
    person_withdraw(param) {
        ...
        axios_zj.post(capital_bl_account_web_sub, rqd).then(data => {
            this.getList();
            message.success("保存成功~");
            this.hideModal();
            this.props.form.resetFields();
    ...
    targetAccountIdChangeEvent(val) {
        ...
        this.timer = setTimeout(() => {
            axios_zj.post(capital_bl_account_user_list, { accountName: val }).then(res => {
    ...
    requestList(param) {
        ...
        return axios_zj.post(capital_bl_account_detail, { ...param })
    }
    render() {
        ...
        return (
            <div>
                ...
                <ListCtrl {...listProps}>
                    <div />
                    <div>
                        <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" type="primary" data-type="cz" onClick={this.showModal.bind(this)}>&emsp;新增加款&emsp;</Permissions>&emsp;
                        <Permissions server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button" type="primary" data-type="tx" onClick={this.showModal.bind(this)}>&emsp;新增提现&emsp;</Permissions>&emsp;
                        <Permissions type="primary" onClick={this.table_export.bind(this)} server={global.AUTHSERVER.account.key} permissions={global.AUTHSERVER.account.access.gj_oper} tag="button">&emsp;导出&emsp;</Permissions>
                    </div>
                </ListCtrl>
                <Modal {...modal_props}>
