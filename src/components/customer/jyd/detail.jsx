...
class Gyl extends Component {
    ...
    componentWillMount() {
        ...
        this.basic = {
            ...
            "qyNo": {
                name: "客户ID",
                render: (e) => { return e.companyNo ? e.companyNo : "--" }
            },
        }
        this.fr = {
            "legalManName": {
    ...
    getImage(storageList=[], des) {
        return storageList.map(storage => ({ src: host_ygd+"/manage/storage_service/get?storageNo="+storage.storageServiceId, des }))
    }
    //订单详情
    getDetail() {
        axios_ygd.get(custom_detail + "?companyId=" + this.state.companyId).then((data) => {
            let idcardArr = [];
