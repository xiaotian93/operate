...
import {jyd_status_log} from '../../ajax/api';
...
class TimeLine extends Component{
    ...
    timeLine() {
        ...
        axios_ygd.post(jyd_status_log,param).then((e)=>{
                                ...
                                <Timeline.Item className="timeline"><span style={{fontSize:"14px"}}>业务发起</span><span className="widths" style={{marginLeft:"30px",color:"#393A3E"}}>订单生成时间：{this.state.data[0].statusChangeTime}</span></Timeline.Item>
                                {
                                    this.state.data.map(function(i,k){
                                        ...
                                        // }else{
                                            ...
                                            // return <Timeline.Item key={k} className="timeline"><span>{status[i.afterStatus]}</span><span className="widths"><p style={{color:"#00B471"}}>{"审批人："+i.accountName}</p><p style={{color:"#00B471"}}>{"审批意见："+(i.comment?i.comment:"无")}</p></span><span className="widths">{i.statusChangeTime}</span></Timeline.Item>
